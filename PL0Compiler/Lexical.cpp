#include "stdafx.h"

enum WORD_TYPE_ENUM {
    INVALID_WORD,
    IDENTIFIER,
    INTEGER,        // 新增：整数类型
    REAL,           // 新增：实数类型
    CONST,
    VAR,
    PROCEDURE,
    BEGIN,
    END,
    IF,
    THEN,
    ELSE,
    WHILE,
    DO,
    WRITE,
    READ,
    CALL,
    LEFT_PARENTHESIS,
    RIGHT_PARENTHESIS,
    COMMA,
    SEMICOLON,
    PERIOD,
    PLUS,
    MINUS,
    MULTIPLY,
    DIVIDE,
    ODD,
    EQL,
    NEQ,
    LES,
    LEQ,
    GTR,
    GEQ,
    ASSIGN
};

// 保留字的名字字符串和类型对照表结构
struct RESERVED_WORD_NAME_VS_TYPE_STRUCT {
    char szName[20];
    enum WORD_TYPE_ENUM eType;
};

#define MAX_LENGTH_OF_A_WORD 10
#define MAX_NUMBER_OF_WORDS 1000
#define NUMBER_OF_RESERVED_WORDS 14

struct WORD_STRUCT {
    char szName[MAX_LENGTH_OF_A_WORD];
    enum WORD_TYPE_ENUM eType;
    double nNumberValue;
    int nLineNo;
};

RESERVED_WORD_NAME_VS_TYPE_STRUCT ReservedWordNameVsTypeTable[NUMBER_OF_RESERVED_WORDS];
WORD_TYPE_ENUM SingleCharacterWordTypeTable[256];
WORD_STRUCT g_Words[MAX_NUMBER_OF_WORDS];
int g_nWordsIndex;
int g_nLineNo = 1;
WORD_TYPE_ENUM g_eLastWordType = INVALID_WORD;
// 当前扫描字符，需在每次词法分析前重置，避免多次运行残留 EOF
static char g_cCurrentChar = SPACE;

#define COMMIT_WORD_AND_ADVANCE() \
    do { \
        g_eLastWordType = g_Words[g_nWordsIndex].eType; \
        g_nWordsIndex++; \
    } while(0)

extern FILE* fpSourceFile;

int WordTypeToString(char string[100], WORD_TYPE_ENUM eWordType);
void PrintInLexis(int nWordsIndex);
int GetAWord();
int SkipComment(char& cACharacter);
void ReportLexicalError(const char* msg);

void InitializeReservedWordTable()
{
    strcpy(ReservedWordNameVsTypeTable[0].szName, "begin"); ReservedWordNameVsTypeTable[0].eType = BEGIN;
    strcpy(ReservedWordNameVsTypeTable[1].szName, "call"); ReservedWordNameVsTypeTable[1].eType = CALL;
    strcpy(ReservedWordNameVsTypeTable[2].szName, "const"); ReservedWordNameVsTypeTable[2].eType = CONST;
    strcpy(ReservedWordNameVsTypeTable[3].szName, "do"); ReservedWordNameVsTypeTable[3].eType = DO;
    strcpy(ReservedWordNameVsTypeTable[4].szName, "end"); ReservedWordNameVsTypeTable[4].eType = END;
    strcpy(ReservedWordNameVsTypeTable[5].szName, "if"); ReservedWordNameVsTypeTable[5].eType = IF;
    strcpy(ReservedWordNameVsTypeTable[6].szName, "odd"); ReservedWordNameVsTypeTable[6].eType = ODD;
    strcpy(ReservedWordNameVsTypeTable[7].szName, "procedure"); ReservedWordNameVsTypeTable[7].eType = PROCEDURE;
    strcpy(ReservedWordNameVsTypeTable[8].szName, "read"); ReservedWordNameVsTypeTable[8].eType = READ;
    strcpy(ReservedWordNameVsTypeTable[9].szName, "then"); ReservedWordNameVsTypeTable[9].eType = THEN;
    strcpy(ReservedWordNameVsTypeTable[10].szName, "else"); ReservedWordNameVsTypeTable[10].eType = ELSE;
    strcpy(ReservedWordNameVsTypeTable[11].szName, "var"); ReservedWordNameVsTypeTable[11].eType = VAR;
    strcpy(ReservedWordNameVsTypeTable[12].szName, "while"); ReservedWordNameVsTypeTable[12].eType = WHILE;
    strcpy(ReservedWordNameVsTypeTable[13].szName, "write"); ReservedWordNameVsTypeTable[13].eType = WRITE;
}

void InitializeSingleCharacterTable()
{
    int i;
    for (i = 0; i <= 255; i++)
    {
        SingleCharacterWordTypeTable[i] = INVALID_WORD;
    }

    SingleCharacterWordTypeTable['+'] = PLUS;
    SingleCharacterWordTypeTable['-'] = MINUS;
    SingleCharacterWordTypeTable['*'] = MULTIPLY;
    SingleCharacterWordTypeTable['/'] = DIVIDE;
    SingleCharacterWordTypeTable['('] = LEFT_PARENTHESIS;
    SingleCharacterWordTypeTable[')'] = RIGHT_PARENTHESIS;
    SingleCharacterWordTypeTable['='] = EQL;
    SingleCharacterWordTypeTable[','] = COMMA;
    SingleCharacterWordTypeTable['.'] = PERIOD;
    SingleCharacterWordTypeTable['#'] = NEQ;
    SingleCharacterWordTypeTable['<'] = LES;
    SingleCharacterWordTypeTable['>'] = GTR;
    SingleCharacterWordTypeTable[';'] = SEMICOLON;
}

int IsUnaryContext(WORD_TYPE_ENUM ePrevType)
{
    switch (ePrevType)
    {
    case INVALID_WORD:
    case ASSIGN:
    case EQL:
    case NEQ:
    case LES:
    case LEQ:
    case GTR:
    case GEQ:
    case PLUS:
    case MINUS:
    case MULTIPLY:
    case DIVIDE:
    case LEFT_PARENTHESIS:
    case COMMA:
    case SEMICOLON:
    case DO:
    case THEN:
    case ELSE:
    case BEGIN:
    case WHILE:
    case IF:
    case VAR:
    case CONST:
    case PROCEDURE:
    case READ:
    case WRITE:
    case CALL:
        return 1;
    default:
        return 0;
    }
}

void LexicalAnalysis()
{
    int nResult;

    InitializeReservedWordTable();
    InitializeSingleCharacterTable();
    fseek(fpSourceFile, 0L, SEEK_SET);

    printf("\n----------------Lexical Analysis Begin!--------------\n");

    g_nWordsIndex = 0;
    g_eLastWordType = INVALID_WORD;
    g_nLineNo = 1;
    g_cCurrentChar = SPACE; // 重置当前字符，避免上一轮停留在 EOF

    nResult = GetAWord();
    while (nResult == OK && g_nWordsIndex >= 1)
    {
        int idx = g_nWordsIndex - 1;
        PrintInLexis(idx);

        // 如果当前单词是非法单词(INVALID_WORD)，报告词法错误并停止进一步分析
        if (g_Words[idx].eType == INVALID_WORD)
        {
            ReportLexicalError("invalid token");
            break;
        }

        nResult = GetAWord();
    }

    printf("----------------Lexical Analysis Finished!--------------\n");
}

char GetACharacterFromFile()
{
    char cACharacter;

    if (fscanf(fpSourceFile, "%c", &cACharacter) == EOF)
        cACharacter = EOF;

    if (cACharacter == RETURN)
        g_nLineNo++;

    return cACharacter;
}

// 新增：跳过注释函数
int SkipComment(char& cACharacter)
{
    if (cACharacter == '/')
    {
        char cNext = GetACharacterFromFile();
        if (cNext == '/') // 单行注释 "//"
        {
            // 跳过直到行尾
            while (cACharacter != RETURN && cACharacter != EOF)
            {
                cACharacter = GetACharacterFromFile();
            }

            cACharacter = GetACharacterFromFile();
            return 1;
        }
        else if (cNext == '*') // 多行注释 "/* ... */"
        {
            cACharacter = GetACharacterFromFile();
            while (cACharacter != EOF)
            {
                if (cACharacter == '*')
                {
                    cACharacter = GetACharacterFromFile();
                    if (cACharacter == '/')
                    {
                        cACharacter = GetACharacterFromFile(); // ��ȡע�ͺ���ַ�
                        return 1;
                    }
                }
                else
                {
                    cACharacter = GetACharacterFromFile();
                }
            }
        }
        else
        {
            // 不是注释，回退一个字符
            fseek(fpSourceFile, -1, SEEK_CUR);
            if (cNext == RETURN) g_nLineNo--; // ������˵��ǻ��з����кż�һ
            cACharacter = '/';
            return 0;
        }
    }

    return 0;
}

int GetAWord()
{
    char& cACharacter = g_cCurrentChar;
    int nAWordIndex;
    int nDigitNumber;
    int i;
    char szAWord[MAX_LENGTH_OF_A_WORD + 1];
    double nNumberValue;
    int bHasDecimalPoint = 0; // 新增：标记是否有小数点
    int bHasExponent = 0;     // 新增：标记是否有指数部分

    // 忽略空格、换行、TAB，并跳过注释
    while ((cACharacter == SPACE || cACharacter == RETURN || cACharacter == TABLE || cACharacter == '/') && cACharacter != EOF)
    {
        if (cACharacter == '/')
        {
            if(!SkipComment(cACharacter))
                break;
        }
        else
        {
            cACharacter = GetACharacterFromFile();
        }
    }

    if (cACharacter != EOF)
    {
        // 新增：处理赋值符号 :=
        if (cACharacter == ':')
        {
            char cNext = GetACharacterFromFile();
            if (cNext == '=')
            {
                g_Words[g_nWordsIndex].eType = ASSIGN;
                strcpy(g_Words[g_nWordsIndex].szName, ":=");
                g_Words[g_nWordsIndex].nLineNo = g_nLineNo;
                COMMIT_WORD_AND_ADVANCE();
                cACharacter = GetACharacterFromFile();
                return OK;
            }
            else
            {
                // 不是赋值符号，回退一个字符
                fseek(fpSourceFile, -1, SEEK_CUR);
                if (cNext == RETURN) g_nLineNo--;

                g_Words[g_nWordsIndex].eType = INVALID_WORD;
                g_Words[g_nWordsIndex].szName[0] = ':';
                g_Words[g_nWordsIndex].szName[1] = 0;
                g_Words[g_nWordsIndex].nLineNo = g_nLineNo;
                COMMIT_WORD_AND_ADVANCE();
                cACharacter = cNext;
                return OK;
            }
        }
        else if (cACharacter >= 'a' && cACharacter <= 'z')
        {
            nAWordIndex = 0;
            do {
                if (nAWordIndex < MAX_LENGTH_OF_A_WORD)
                    szAWord[nAWordIndex++] = cACharacter;

                cACharacter = GetACharacterFromFile();
            } while ((cACharacter >= 'a' && cACharacter <= 'z' || cACharacter >= '0' && cACharacter <= '9' || cACharacter == '_') && cACharacter != EOF);

            if (cACharacter != EOF)
            {
                szAWord[nAWordIndex] = 0;

                for (i = 0; i < NUMBER_OF_RESERVED_WORDS; i++)
                    if (strcmp(szAWord, ReservedWordNameVsTypeTable[i].szName) == 0)
                    {
                        g_Words[g_nWordsIndex].eType = ReservedWordNameVsTypeTable[i].eType;
                        break;
                    }
                if (i >= NUMBER_OF_RESERVED_WORDS)
                    g_Words[g_nWordsIndex].eType = IDENTIFIER;

                strcpy(g_Words[g_nWordsIndex].szName, szAWord);
                g_Words[g_nWordsIndex].nLineNo = g_nLineNo;

                COMMIT_WORD_AND_ADVANCE();

                return OK;
            }
            else
                return ERROR;
        }
        else if ((cACharacter >= '0' && cACharacter <= '9') || cACharacter == '+' || cACharacter == '-' || cACharacter == '.')
        {
            int bIsNumber = 0;
            nDigitNumber = 0;
            bHasDecimalPoint = 0;
            bHasExponent = 0;

            int bAllowUnarySign = IsUnaryContext(g_eLastWordType);

            // 处理可选的正负号
            if (cACharacter == '+' || cACharacter == '-')
            {
                if (bAllowUnarySign)
                {
                    char cSign = cACharacter;
                    cACharacter = GetACharacterFromFile();
                    if ((cACharacter >= '0' && cACharacter <= '9') || cACharacter == '.')
                    {
                        szAWord[nDigitNumber++] = cSign;
                        bIsNumber = 1;
                    }
                    else
                    {
                        g_Words[g_nWordsIndex].eType = (cSign == '+') ? PLUS : MINUS;
                        g_Words[g_nWordsIndex].szName[0] = cSign;
                        g_Words[g_nWordsIndex].szName[1] = 0;
                        g_Words[g_nWordsIndex].nLineNo = g_nLineNo;

                        COMMIT_WORD_AND_ADVANCE();

                        return OK;
                    }
                }
                else
                {
                    g_Words[g_nWordsIndex].eType = (cACharacter == '+') ? PLUS : MINUS;
                    g_Words[g_nWordsIndex].szName[0] = cACharacter;
                    g_Words[g_nWordsIndex].szName[1] = 0;
                    g_Words[g_nWordsIndex].nLineNo = g_nLineNo;

                    COMMIT_WORD_AND_ADVANCE();

                    cACharacter = GetACharacterFromFile();

                    return OK;
                }
            }

            // 处理以小数点起始的形式 .d+
            if (cACharacter == '.')
            {
                char cNext = GetACharacterFromFile();
                if (cNext >= '0' && cNext <= '9')
                {
                    szAWord[nDigitNumber++] = '0';
                    szAWord[nDigitNumber++] = '.';
                    szAWord[nDigitNumber++] = cNext;
                    cACharacter = GetACharacterFromFile();
                    bHasDecimalPoint = 1; // 标记有小数点
                    while (cACharacter >= '0' && cACharacter <= '9' && cACharacter != EOF)
                    {
                        szAWord[nDigitNumber++] = cACharacter;
                        cACharacter = GetACharacterFromFile();
                    }
                    bIsNumber = 1;
                }
                else
                {
                    g_Words[g_nWordsIndex].eType = PERIOD;
                    g_Words[g_nWordsIndex].szName[0] = '.';
                    g_Words[g_nWordsIndex].szName[1] = 0;
                    g_Words[g_nWordsIndex].nLineNo = g_nLineNo;

                    COMMIT_WORD_AND_ADVANCE();
                    cACharacter = cNext;
                    return OK;
                }
            }

            // 处理数字起始的部分 d+( .d* )?
            if (cACharacter >= '0' && cACharacter <= '9')
            {
                bIsNumber = 1;
                do {
                    szAWord[nDigitNumber++] = cACharacter;
                    cACharacter = GetACharacterFromFile();
                } while (cACharacter >= '0' && cACharacter <= '9' && cACharacter != EOF);

                if (cACharacter == '.')
                {
                    szAWord[nDigitNumber++] = cACharacter;
                    cACharacter = GetACharacterFromFile();
                    bHasDecimalPoint = 1; // 标记有小数点
                    while (cACharacter >= '0' && cACharacter <= '9' && cACharacter != EOF)
                    {
                        szAWord[nDigitNumber++] = cACharacter;
                        cACharacter = GetACharacterFromFile();
                    }
                }
            }

            // 处理科学计数法部分 [eE][+-]?[0-9]+
            if (bIsNumber && (cACharacter == 'e' || cACharacter == 'E'))
            {
                szAWord[nDigitNumber++] = cACharacter;
                cACharacter = GetACharacterFromFile();
                bHasExponent = 1; // 标记有指数部分

                // 处理科学计数法中的可选符号
                if (cACharacter == '+' || cACharacter == '-')
                {
                    szAWord[nDigitNumber++] = cACharacter;
                    cACharacter = GetACharacterFromFile();
                }

                // 科学计数法后必须有数字
                if (cACharacter >= '0' && cACharacter <= '9')
                {
                    do {
                        szAWord[nDigitNumber++] = cACharacter;
                        cACharacter = GetACharacterFromFile();
                    } while (cACharacter >= '0' && cACharacter <= '9' && cACharacter != EOF);
                }
                else
                {
                    // 科学计数法后没有数字，格式错误
                    nDigitNumber--;
                    szAWord[nDigitNumber] = 0;
                }
            }

            if (bIsNumber)
            {
                szAWord[nDigitNumber] = 0;

                // 判断是整数还是实数
                if (bHasDecimalPoint || bHasExponent)
                {
                    g_Words[g_nWordsIndex].eType = REAL; // 实数类型
                }
                else
                {
                    g_Words[g_nWordsIndex].eType = INTEGER; // 整数类型
                }

                strcpy(g_Words[g_nWordsIndex].szName, szAWord);
                {
                    double d = 0.0;
                    sscanf(szAWord, "%lf", &d);
                g_Words[g_nWordsIndex].nNumberValue = d;
                }
                g_Words[g_nWordsIndex].nLineNo = g_nLineNo;

                COMMIT_WORD_AND_ADVANCE();

                return OK;
            }
        }
        else if (cACharacter == '>')
        {
            cACharacter = GetACharacterFromFile();
            if (cACharacter == '=')
            {
                g_Words[g_nWordsIndex].eType = GEQ;
                strcpy(g_Words[g_nWordsIndex].szName, ">=");
                g_Words[g_nWordsIndex].nLineNo = g_nLineNo;

                COMMIT_WORD_AND_ADVANCE();

                cACharacter = GetACharacterFromFile();
            }
            else
            {
                g_Words[g_nWordsIndex].eType = GTR;
                strcpy(g_Words[g_nWordsIndex].szName, ">");
                g_Words[g_nWordsIndex].nLineNo = g_nLineNo;

                COMMIT_WORD_AND_ADVANCE();
            }
            return OK;
        }
        else if (cACharacter == '<')
        {
            cACharacter = GetACharacterFromFile();
            if (cACharacter == '=')
            {
                g_Words[g_nWordsIndex].eType = LEQ;
                strcpy(g_Words[g_nWordsIndex].szName, "<=");
                g_Words[g_nWordsIndex].nLineNo = g_nLineNo;

                COMMIT_WORD_AND_ADVANCE();

                cACharacter = GetACharacterFromFile();
            }
            else
            {
                g_Words[g_nWordsIndex].eType = LES;
                strcpy(g_Words[g_nWordsIndex].szName, "<");
                g_Words[g_nWordsIndex].nLineNo = g_nLineNo;

                COMMIT_WORD_AND_ADVANCE();
            }
            return OK;
        }
        else
        {
            g_Words[g_nWordsIndex].eType = SingleCharacterWordTypeTable[cACharacter];
            g_Words[g_nWordsIndex].szName[0] = cACharacter;
            g_Words[g_nWordsIndex].szName[1] = 0;
            g_Words[g_nWordsIndex].nLineNo = g_nLineNo;

            COMMIT_WORD_AND_ADVANCE();

            cACharacter = GetACharacterFromFile();

            return OK;
        }
    }
    return ERROR;
}

void PrintInLexis(int nIndex)
{
    char szWordName[100];
    char szWordType[100];

    strcpy(szWordName, g_Words[nIndex].szName);
    WordTypeToString(szWordType, g_Words[nIndex].eType);

    switch (g_Words[nIndex].eType)
    {
    case IDENTIFIER:
        printf("%-4d%-15s%-20s%-15s\n", nIndex, szWordName, szWordType, szWordName);
        break;
    case INTEGER:  // 新增：整数类型输出
        printf("%-4d%-15s%-20s%-10.0f\n", nIndex, szWordName, szWordType, g_Words[nIndex].nNumberValue);
        break;
    case REAL:     // 新增：实数类型输出
        printf("%-4d%-15s%-20s%-10g\n", nIndex, szWordName, szWordType, g_Words[nIndex].nNumberValue);
        break;
    default:
    {
        int len = strlen(g_Words[nIndex].szName);
        if (len == 1) {
            printf("%-4d%-15s%-20s%d\n", nIndex, szWordName, szWordType, (int)g_Words[nIndex].szName[0]);
        }
        else {
            printf("%-4d%-15s%-20s%-15s\n", nIndex, szWordName, szWordType, szWordName);
        }
    }
    }
}

// 统一的词法错误报告函数：打印当前单词序号、行号和内容
void ReportLexicalError(const char* msg)
{
    if(g_nWordsIndex <= 0)
        return;
    int idx = g_nWordsIndex - 1;
    printf("\nLEXICAL ERROR at token %d (line %d, '%s'): %s\n",
           idx,
           g_Words[idx].nLineNo,
           g_Words[idx].szName,
           msg);
}

int WordTypeToString(char strString[100], WORD_TYPE_ENUM eWordType)
{
    switch (eWordType)
    {
    case IDENTIFIER: strcpy(strString, "IDENTIFIER"); break;
    case INTEGER: strcpy(strString, "INTEGER"); break;      // 新增
    case REAL: strcpy(strString, "REAL"); break;           // 新增
    case PLUS: strcpy(strString, "PLUS"); break;
    case MULTIPLY: strcpy(strString, "MULTIPLY"); break;
    case MINUS: strcpy(strString, "MINUS"); break;
    case DIVIDE: strcpy(strString, "DIVIDE"); break;
    case LES: strcpy(strString, "LES"); break;
    case LEQ: strcpy(strString, "LEQ"); break;
    case GTR: strcpy(strString, "GTR"); break;
    case GEQ: strcpy(strString, "GEQ"); break;
    case EQL: strcpy(strString, "EQL"); break;
    case NEQ: strcpy(strString, "NEQ"); break;
    case LEFT_PARENTHESIS: strcpy(strString, "LEFT_PARENTHESIS"); break;
    case RIGHT_PARENTHESIS: strcpy(strString, "RIGHT_PARENTHESIS"); break;
    case COMMA: strcpy(strString, "COMMA"); break;
    case SEMICOLON: strcpy(strString, "SEMICOLON"); break;
    case PERIOD: strcpy(strString, "PERIOD"); break;
    case ASSIGN: strcpy(strString, "ASSIGN"); break;
    case CONST: strcpy(strString, "CONST"); break;
    case VAR: strcpy(strString, "VAR"); break;
    case PROCEDURE: strcpy(strString, "PROCEDURE"); break;
    case BEGIN: strcpy(strString, "BEGIN"); break;
    case END: strcpy(strString, "END"); break;
    case IF: strcpy(strString, "IF"); break;
    case THEN: strcpy(strString, "THEN"); break;
    case ELSE: strcpy(strString, "ELSE"); break;
    case ODD: strcpy(strString, "ODD"); break;
    case WHILE: strcpy(strString, "WHILE"); break;
    case DO: strcpy(strString, "DO"); break;
    case CALL: strcpy(strString, "CALL"); break;
    case READ: strcpy(strString, "READ"); break;
    case WRITE: strcpy(strString, "WRITE"); break;
    case INVALID_WORD: strcpy(strString, "INVALID_WORD"); break;
    }
    return NULL;
}
